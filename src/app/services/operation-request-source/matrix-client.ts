...
import { switchMap, first, publishReplay, refCount, filter, withLatestFrom, map } from 'rxjs/operators';
...
export class MatrixClient {
  public connect$: Observable<any> = defer(() => {
    ...
    return new Observable<any>((sub) => {
      ...
      (async () => {
        try {
          const response = await client.login('m.login.password', { user: await this.getUser(), password: await this.getPassword() })
  ...
  private createSigningMessage() {
    return Buffer.from(`login:${Math.floor(Date.now() / (5 * 60000))}`, 'utf8').toString('hex');
  }

  private async getUser() {
    const hexPublicKey = taquitoUtil.b58cdecode(await this.taquito.signer.publicKey(), taquitoUtil.prefix['edpk']);
    const hexPubkeyHash = taquitoUtil.buf2hex((toBuffer(sodium.crypto_generichash(32, hexPublicKey))))
    return `@${hexPubkeyHash}:matrix.papers.tech`;
  }

  private async getPassword() {
    // TODO: Allow passing now magic bytes
    const { sig } = await (this.taquito.signer as any).sign(this.createSigningMessage());
      ...
      switchMap((client) => {
        return new Observable<string>((sub) => {
          client.createRoom({
            ...
            setTimeout(() => {
              client.sendTextMessage(room_id, '@channel-open:a0b28a06497f357ef7b27b60a0ceedb5e3fd7eeb029f669d5faef65ecff6a992', console.log);
            }, 5000);
